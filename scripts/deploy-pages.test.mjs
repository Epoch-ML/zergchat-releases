...
const PAGE_URL = "https://epoch-ml.github.io/zergchat-releases/";
...
test("queued Pages deployment can succeed after the upstream ten-minute ceiling", async () => {
  ...
  assert.equal(
    createRequest.url,
    "https://api.github.test/repos/Epoch-ML/zergchat-releases/pages/deployments",
  );
  assert.deepEqual(createRequest.headers, {
    Accept: "application/vnd.github+json",
    Authorization: "Bearer github-token",
    "Content-Type": "application/json",
    "User-Agent": "zergchat-pages-deployer",
  ...
  assert.equal(
    ...
    `https://api.github.test/repos/Epoch-ML/zergchat-releases/pages/deployments/${DEPLOYMENT_ID}`,
  );
...
test("workflow environment writes the exact deployed HTTPS root to its output", async () => {
  const directory = await mkdtemp(join(tmpdir(), "zergchat-pages-output-"));
  ...
  try {
    const result = await runPagesDeploymentFromEnvironment(
      {
        ...
        GITHUB_REPOSITORY: "Epoch-ML/zergchat-releases",
    ...
    assert.equal(
      ...
      "https://api.github.com/repos/Epoch-ML/zergchat-releases/pages/deployments",
    );
  } finally {
