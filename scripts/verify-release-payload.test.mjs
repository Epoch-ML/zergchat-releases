...
async function makePayload(label, payloadRequest = request()) {
  const root = await mkdtemp(join(tmpdir(), "zergchat-release-payload-"));
...
describe("canonical Zergchat release payload verification", () => {
  ...
  it("validates every release-metadata trust and integrity field", async () => {
    ...
    const stableRequest = {
      ...
      sourceRef: "refs/tags/zergchat-v0.2.0",
      version: "0.2.0",
    };
