...
const failedArtifactDetails = `Executable=/private/tmp/Zergchat.app/Contents/MacOS/app
...
async function makeTemporaryDirectory() {
  const directory = await mkdtemp(join(tmpdir(), "zergchat-source-signature-"));
  temporaryDirectories.push(directory);
  return directory;
}

describe("Zergchat source signature policy", () => {
  ...
  it("accepts only codesign's explicit unsigned failure", () => {
    assert.deepEqual(
      verifySourceSignature(
        "/private/tmp/Zergchat.app: code object is not signed at all\n",
        255,
      ),
      { kind: "unsigned" },
    );
