...
async function writeRawTar(archivePath, entries) {
  ...
  for (const entry of entries) {
    ...
    const header = new Header({
      ...
      path: entry.path.length > 100 ? "Zergchat.app/pax-placeholder" : entry.path,
      size,
...
async function makeApplication() {
  const root = await mkdtemp(join(tmpdir(), "zergchat-macos-package-"));
  temporaryDirectories.push(root);
  const applicationPath = join(root, "bundle", "Zergchat.app");
  const executablePath = join(applicationPath, "Contents", "MacOS", "Zergchat");
...
describe("deterministic macOS updater packaging", () => {
  it("produces identical archives with a complete sorted app tree", async () => {
    ...
    assert.deepEqual(names, [
      "Zergchat.app/",
      "Zergchat.app/Contents/",
      "Zergchat.app/Contents/MacOS/",
      "Zergchat.app/Contents/MacOS/Zergchat",
      "Zergchat.app/Contents/Resources/",
      "Zergchat.app/Contents/Resources/version.txt",
    ]);
    assert.deepEqual(
      [...new Set(mtimes)],
  ...
  it("extracts one bounded regular app tree without executing payload files", async () => {
    const fixture = await makeApplication();
    await rm(join(fixture.applicationPath, "Contents", "version-link"));
    const archivePath = join(fixture.root, "Zergchat.source.app.tar.gz");
    ...
    assert.equal(result.applicationPath, join(extractionRoot, "Zergchat.app"));
    assert.equal(
      await readFile(join(result.applicationPath, "Contents", "MacOS", "Zergchat"), "utf8"),
      "#!/bin/sh\nexit 0\n",
    );
  ...
  it("rejects every non-contained archive member path and a missing explicit root", async () => {
    ...
    for (const [index, unsafePath] of unsafePaths.entries()) {
      ...
      await writeRawTar(archivePath, [
        { path: "Zergchat.app/", type: "Directory" },
    ...
    await assert.rejects(
      ...
      /archive must contain exactly one Zergchat\.app root/,
    );

    const longPathArchive = join(fixture.root, "long-path.app.tar.gz");
    const longPath = "Zergchat.app/" + "a".repeat(4_084);
  ...
  it("rejects duplicate archive members", async () => {
    ...
    await writeRawTar(duplicateArchive, [
      { path: "Zergchat.app/", type: "Directory" },
      { contents: "first", path: "Zergchat.app/value" },
      { contents: "second", path: "Zergchat.app/value" },
    ]);
    await assert.rejects(
      extractSourceApplication({
      ...
      /archive contains a duplicate path: Zergchat\.app\/value/,
    );

    const fileRootArchive = join(fixture.root, "file-root.app.tar.gz");
    await writeRawTar(fileRootArchive, [
      { contents: "not an application directory", path: "Zergchat.app" },
    ]);
    await assert.rejects(
      extractSourceApplication({
      ...
      /archive Zergchat\.app root must be a directory/,
    );
  ...
  it("keeps extraction outside the archive and removes a partially extracted tree", async () => {
    ...
    await writeRawTar(conflictingArchive, [
      { path: "Zergchat.app/", type: "Directory" },
      { contents: "file", path: "Zergchat.app/parent" },
      { contents: "child", path: "Zergchat.app/parent/child" },
    ]);
    ...
    await assert.rejects(
      ...
      /archive path hierarchy conflicts at Zergchat\.app\/parent/,
    );
