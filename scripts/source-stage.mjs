...
function assertMetadata(metadata, request) {
  if (metadata?.schema_version !== 2 || metadata.product !== "Zergchat") {
...
export async function prepareSourceStage(options) {
  ...
  if (basename(extraction.applicationPath) !== "Zergchat.app") {
    throw new Error("source-stage archive did not extract one Zergchat.app");
  }
