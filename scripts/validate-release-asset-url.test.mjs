...
const validInput = {
  assetUrl:
    "https://github.com/Epoch-ML/zergchat-releases/releases/download/zergchat-v0.2.0/ZERGCHAT_0.2.0_aarch64.app.tar.gz",
  assetName: "ZERGCHAT_0.2.0_aarch64.app.tar.gz",
...
describe("release asset URL validation", () => {
  it("accepts only the canonical protected-tag URL shape", () => {
    assert.deepEqual(validateReleaseAssetUrl(validInput), {
      owner: "Epoch-ML",
      repository: "zergchat-releases",
      release: "zergchat-v0.2.0",
      asset: "ZERGCHAT_0.2.0_aarch64.app.tar.gz",
    });
    assert.deepEqual(
  ...
  it("rejects credentials, metadata, and path substitutions", () => {
    const invalidInputs = [
      ...
      { assetUrl: validInput.assetUrl.replace("zergchat-v0.2.0", "untagged-deadbeef") },
      ...
      {
        assetUrl: validInput.assetUrl.replace("zergchat-releases", ""),
        repository: "Epoch-ML/",
      },
      { repository: "Epoch-ML/zergchat-releases/extra" },
      { assetName: "different.tar.gz" },
      { releaseTag: "zergchat-v9.9.9" },
      { releaseIsDraft: "false" },
    ];
