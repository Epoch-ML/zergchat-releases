...
describe("Zergchat release workflow contract", () => {
  it("serializes releases and validates one immutable request", () => {
    ...
    assert.match(workflow, /group: zergchat-v2-release/);
  ...
  it("checks out exact source with authenticated host metadata", () => {
    ...
    assert.match(workflow, /source_commit_requested_at.*ZERGCHAT_RELEASE_DATE/s);
  ...
  it("keeps exact-source gates free of release metadata and scopes later inputs", () => {
    ...
    const releaseVariables = [
      ...
      "ZERGCHAT_RELEASE_DATE",
      "ZERGCHAT_RELEASE_REPOSITORY",
      "ZERGCHAT_RELEASE_TAG",
      "ZERGCHAT_SOURCE_SHA",
      "ZERGCHAT_UPDATE_BASE_URL",
    ...
    assert.match(
      timestampStep,
      /ZERGCHAT_RELEASE_DATE: \$\{\{ needs\.validate\.outputs\.requested_at \}\}/,
    );
    ...
    for (const binding of [
      "ZERGCHAT_DESKTOP_VERSION: ${{ needs.validate.outputs.version }}",
      "ZERGCHAT_RELEASE_TAG: ${{ needs.validate.outputs.release_tag }}",
      "ZERGCHAT_SOURCE_SHA: ${{ needs.validate.outputs.source_sha }}",
      "ZERGCHAT_UPDATE_CHANNEL: ${{ needs.validate.outputs.channel }}",
    ]) {
  ...
  it("recovers published immutable retries from canonical public bytes", () => {
    ...
    assert.match(publishJob, /name: zergchat-v2-canonical-release/);
    assert.match(feedJob, /name: zergchat-v2-canonical-release/);
    assert.match(pagesJob, /name: zergchat-v2-canonical-release/);
    assert.doesNotMatch(
      feedJob,
      /name: zergchat-v2-macos/,
    ...
    const canonicalDownload = publishJob.slice(
      ...
      publishJob.indexOf("name: zergchat-v2-canonical-release"),
    );
    assert.match(
      canonicalDownload,
  ...
  it("executes every single-quoted jq program in the workflow", async () => {
    ...
    const asset = {
      browser_download_url:
        "https://github.com/Epoch-ML/zergchat-releases/releases/download/example/checksums.txt",
      digest: `sha256:${"a".repeat(64)}`,
      id: 17,
      name: "checksums.txt",
      size: 312,
    ...
    const document = {
      ...
      status: "Accepted",
      tag_name: "zergchat-preview-v0.2.0-preview.3",
    };
  ...
  it("isolates updater signing from private source and pins GitHub-owned actions", () => {
    ...
    const signingSecretUses = workflow.match(
      /secrets\.ZERGCHAT_(?:PREVIEW|STABLE)_TAURI_SIGNING_PRIVATE_KEY(?:_PASSWORD)?/g,
    ) ?? [];
  ...
  it("uses fresh source, Apple, and updater hosts with disjoint credentials", () => {
    ...
    assert.match(buildJob, /zergchat-(stable|preview)-build/);
    assert.match(buildJob, /--no-sign/);
    assert.doesNotMatch(buildJob, /ZERGCHAT_APPLE_|TAURI_SIGNING_PRIVATE_KEY|codesign|notarytool/);
    assert.match(appleJob, /zergchat-apple-/);
  ...
  it("binds every channel to a distinct embedded and signing trust root", () => {
    assert.match(workflow, /keys\/zergchat-preview-updater\.pubkey/);
    assert.match(workflow, /keys\/zergchat-stable-updater\.pubkey/);
    assert.match(workflow, /updater-preview\.pubkey/);
    assert.match(workflow, /updater-stable\.pubkey/);
    assert.doesNotMatch(workflow, /keys\/zergchat-updater-v2\.pubkey/);
  ...
  it("treats source archives as hostile before Apple credentials are imported", () => {
    ...
    const appleSecrets = workflow.indexOf(
      "ZERGCHAT_APPLE_CERTIFICATE: ${{ secrets.ZERGCHAT_APPLE_CERTIFICATE }}",
    );
