...
async function makeFixture(metadataOverrides = {}) {
  const root = await mkdtemp(join(tmpdir(), "zergchat-source-stage-"));
  temporaryDirectories.push(root);
  const applicationPath = join(root, "application", "Zergchat.app");
  ...
  const executable = join(applicationPath, "Contents", "MacOS", "Zergchat");
...
describe("hostile Zergchat source stage", () => {
  it("binds one unsigned app archive to the immutable request and channel key", async () => {
    ...
    assert.equal(result.applicationPath, join(fixture.outputDirectory, "Zergchat.app"));
    assert.equal(
