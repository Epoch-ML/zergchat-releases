...
function normalizeArchivePath(path) {
  if (
    ...
    throw new Error("archive path must remain under Zergchat.app/");
  }
  const withoutTrailingSlash = path.endsWith("/") ? path.slice(0, -1) : path;
  const segments = withoutTrailingSlash.split("/");
  if (
    segments.length === 0 ||
    segments[0] !== "Zergchat.app" ||
...
function makeArchiveValidator(budgets) {
  ...
  return {
    ...
    result() {
      if (!seen.has("Zergchat.app")) {
        throw new Error("archive must contain exactly one Zergchat.app root");
      }
      if (entryTypes.get("Zergchat.app") !== "directory") {
        throw new Error("archive Zergchat.app root must be a directory");
      }
      return { ...state };
    },
  };
...
async function inspectExtractedTree(root, budgets) {
  ...
  if (basename(root) !== "Zergchat.app") {
    throw new Error("extracted application root must be Zergchat.app");
  }
  return result;
}
...
export async function extractSourceApplication(options) {
  ...
  try {
    ...
    const applicationPath = join(outputDirectory, "Zergchat.app");
