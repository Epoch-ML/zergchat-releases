...
async function makeTemporaryDirectory() {
  const directory = await mkdtemp(join(tmpdir(), "zergchat-release-resolution-"));
...
describe("GitHub draft release resolution", () => {
  it("selects one exact draft tag across paginated results", () => {
    ...
    const pages = [
      ...
      [draft, { id: 2, tag_name: "zergchat-v0.1.0", draft: false }],
    ];

    assert.deepEqual(resolveReleaseByTag(pages, draft.tag_name), draft);
    assert.equal(resolveReleaseByTag(pages, "zergchat-preview-v9.9.9"), null);
  });

  it("selects an exact published release with the same contract", () => {
    const published = {
      id: 42,
      tag_name: "zergchat-v1.2.3",
      draft: false,
      prerelease: false,
      immutable: true,
    };
  ...
  it("fails closed when more than one release has the exact tag", () => {
    const tag = "zergchat-preview-v0.2.0-preview.3";
    assert.throws(
      () => resolveReleaseByTag([
        [{ id: 1, tag_name: tag }],
        [{ id: 2, tag_name: tag }],
  ...
  it("rejects malformed pages, entries, and tags", () => {
    for (const pages of [null, {}, [], [[]]]) {
      ...
      assert.throws(
        () => resolveReleaseByTag(pages, "zergchat-v1.0.0"),
        /array of API pages/,
      );
    }
    for (const entry of [null, [], "release", 7, {}]) {
      assert.throws(
        () => resolveReleaseByTag([[entry]], "zergchat-v1.0.0"),
        /non-object entry|missing tag_name/,
      );
    }
    for (const tag of [null, "", "zergchat-v1.0.0\n", 7]) {
      assert.throws(
